  ...
  return (
    <div className="page-container" style={{ paddingBottom: '2rem' }}>
      ...
      <div className="tab-group">
        {[['stats', '📈 Stats'], ['memory', '🖼️ Memory Bank'], ['reminders', '⏰ Reminders']].map(([key, label]) => (
      ...
      {tab === 'stats' && (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
          <div className="grid-4 mb-3">
            ...
            <div className="stat-card">
              <div className="stat-value" style={{ background: 'linear-gradient(135deg, #F59E0B, #FB923C)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>🔥 {p.streak}</div>
              <div className="stat-label">Streak</div>
            </div>
            <div className="stat-card">
              <div className="stat-value" style={{ color: declineColors[data.decline_status?.status], fontSize: '1rem' }}>
          ...
          <div className="chart-container">
            <h3 className="mb-2">📈 Accuracy Trend</h3>
            <ResponsiveContainer width="100%" height={250}>
              <LineChart data={data.accuracy_trend}>
                ...
                <Line type="monotone" dataKey="accuracy" stroke="#6366F1" strokeWidth={3} dot={{ r: 4, fill: '#6366F1' }} />
              </LineChart>
            </ResponsiveContainer>
          </div>
      ...
      {tab === 'memory' && (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
          <button className="btn btn-primary w-full mb-3" onClick={() => setShowUpload(!showUpload)}>
          ...
          {showUpload && (
            <motion.div className="glass-card mb-3" initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
              ...
              <button className="btn btn-success w-full" onClick={handleUploadMemory}>✅ Save</button>
            </motion.div>
          )}

          <div className="grid-3">
            {memories.map(m => (
              <div key={m.id} className="glass-card" style={{ padding: '0.75rem', textAlign: 'center' }}>
