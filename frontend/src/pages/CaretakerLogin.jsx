...
export default function CaretakerLogin() {
  ...
  useEffect(() => {
    ...
    tl.fromTo('.ctlog-left', { opacity: 0, x: -60 }, { opacity: 1, x: 0, duration: 0.9 })
      .fromTo('.ctlog-right', { opacity: 0, x: 60 }, { opacity: 1, x: 0, duration: 0.9 }, '-=0.7')
      .fromTo('.ctlog-field', { opacity: 0, y: 20 }, { opacity: 1, y: 0, duration: 0.5, stagger: 0.1 }, '-=0.4');
    gsap.to('.ctlog-orb-1', { x: 15, y: -20, repeat: -1, yoyo: true, duration: 5, ease: 'power1.inOut' });
    gsap.to('.ctlog-orb-2', { x: -20, y: 15, repeat: -1, yoyo: true, duration: 7, ease: 'power1.inOut', delay: 1 });
    gsap.to('.ctlog-heart', { scale: 1.15, repeat: -1, yoyo: true, duration: 0.8, ease: 'power1.inOut' });
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#06060e', display: 'flex', overflow: 'hidden', position: 'relative' }}>
      <div className="ctlog-orb-1" style={{ position: 'fixed', top: '-10%', right: '-5%', width: 500, height: 500, borderRadius: '50%', background: 'radial-gradient(circle, rgba(14,165,233,0.12) 0%, transparent 70%)', pointerEvents: 'none' }} />
      <div className="ctlog-orb-2" style={{ position: 'fixed', bottom: '-15%', left: '-10%', width: 400, height: 400, borderRadius: '50%', background: 'radial-gradient(circle, rgba(109,40,217,0.1) 0%, transparent 70%)', pointerEvents: 'none' }} />
      <div style={{ position: 'fixed', inset: 0, backgroundImage: 'linear-gradient(rgba(14,165,233,0.02) 1px, transparent 1px), linear-gradient(90deg, rgba(14,165,233,0.02) 1px, transparent 1px)', backgroundSize: '60px 60px', pointerEvents: 'none' }} />

      {/* Left panel */}
      <div className="ctlog-left" style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '4rem', position: 'relative', opacity: 0 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '2rem' }}>
          <div style={{ width: 60, height: 60, borderRadius: '16px', background: 'linear-gradient(135deg, #0ea5e9, #6d28d9)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 24px rgba(14,165,233,0.4)', position: 'relative' }}>
            <UserCheck size={30} color="white" />
            <div className="ctlog-heart" style={{ position: 'absolute', top: -6, right: -6, width: 20, height: 20, borderRadius: '50%', background: '#f87171', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <Heart size={11} color="white" fill="white" />
            </div>
          </div>
          <div>
            <div style={{ fontFamily: 'Outfit, sans-serif', fontSize: '1.8rem', fontWeight: 900, color: '#f0f0f5' }}>Caretaker Portal</div>
            <div style={{ fontSize: '0.85rem', color: '#0ea5e9', fontWeight: 600 }}>Patient Care & Monitoring</div>
          </div>
        </div>
        <h1 style={{ fontFamily: 'Outfit, sans-serif', fontSize: 'clamp(2rem, 3vw, 3rem)', fontWeight: 900, color: '#f0f0f5', lineHeight: 1.1, marginBottom: '1.25rem' }}>
          Care with<br />
          <span style={{ background: 'linear-gradient(135deg, #0ea5e9, #6d28d9)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>confidence.</span>
        </h1>
        <p style={{ color: '#5a5a72', fontSize: '1rem', lineHeight: 1.7, maxWidth: 400, marginBottom: '2.5rem' }}>
          Receive real-time patient health reports, monitor cognitive trends, and manage daily care activities efficiently.
        </p>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
          {['Real-time patient health reports', 'Cognitive trend analytics', 'Smart care reminders', 'Critical alert notifications'].map((item, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
              <div style={{ width: 6, height: 6, borderRadius: '50%', background: '#0ea5e9', boxShadow: '0 0 8px rgba(14,165,233,0.6)' }} />
      ...
      {/* Right panel - Form */}
      <div className="ctlog-right" style={{ width: '45%', minWidth: 420, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem', opacity: 0, perspective: 1000 }}>
        <div className="ctlog-card-inner" style={{ width: '100%', maxWidth: 440, transformStyle: 'preserve-3d' }}>
          <div style={{ position: 'relative', background: 'rgba(14,14,26,0.8)', backdropFilter: 'blur(30px)', border: '1px solid rgba(14,165,233,0.2)', borderRadius: '24px', padding: '2.5rem', boxShadow: '0 8px 60px rgba(14,165,233,0.12)' }}>
            <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 3, borderRadius: '24px 24px 0 0', background: 'linear-gradient(135deg, #0ea5e9, #6d28d9)' }} />
            ...
            <form className="ctlog-form" onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              ...
              <button type="submit" disabled={loading} style={{ marginTop: '0.5rem', padding: '0.9rem', borderRadius: '10px', border: 'none', background: 'linear-gradient(135deg, #0ea5e9, #6d28d9)', color: 'white', fontSize: '1rem', fontWeight: 700, fontFamily: 'Inter, sans-serif', cursor: loading ? 'not-allowed' : 'pointer', opacity: loading ? 0.7 : 1, transition: 'all 0.3s', boxShadow: '0 4px 20px rgba(14,165,233,0.3)' }}
      ...
      <style>{`@keyframes spin { to { transform: rotate(360deg); } } @media(max-width:768px){.ctlog-left{display:none}.ctlog-right{width:100%!important;min-width:unset!important}}`}</style>
    </div>
  );
}
